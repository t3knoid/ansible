...
| [`prometheus/deploy_blackbox_exporter.yml`](./deploy_blackbox_exporter.md) | Deploy Prometheus Blackbox Exporter on a selected host group for remote service probing |
| [`prometheus/deploy_nginx_prometheus_exporter.yml`](./deploy_nginx_prometheus_exporter.md) | Deploy nginx-prometheus-exporter on nginx exporter hosts Usage: ansible-playbook -i inventory/rproxy/inventory.ini playbooks/prometheus/deploy_nginx_prometheus_exporter.yml |
| [`prometheus/deploy_node_exporter.yml`](./deploy_node_exporter.md) | Deploy Node Exporter on node_exporter hosts Note: This playbook only installs or updates node_exporter on the selected inventory's node_exporter hosts. It does not update Prometheus scrape targets. |
| [`prometheus/deploy_postgres_exporter.yml`](./deploy_postgres_exporter.md) | Deploy postgres_exporter on a selected host group for PostgreSQL monitoring |
| [`prometheus/deploy_prometheus.yml`](./deploy_prometheus.md) | Deploy Prometheus monitoring system Usage: ansible-playbook -i inventory/prometheus/inventory.ini playbooks/prometheus/deploy_prometheus.yml Use playbooks/prometheus/deploy_prometheus_exporters.yml to refresh exporter scrape targets. |
| [`prometheus/deploy_prometheus_exporters.yml`](./deploy_prometheus_exporters.md) | Refresh Prometheus exporter scrape targets Usage: ansible-playbook playbooks/prometheus/deploy_prometheus_exporters.yml Note: This refreshes exporter targets from the current inventory into the Prometheus config. Requires the full combined inventory (ansible.cfg's default inventory=inventory/) so every service's exporter group (node_exporter, postgres_exporter, pve_exporter, etc.) is visible. Do not scope this run with -i to a single inventory subfolder. |
...
| [`proxmox/deploy_pve_monitoring.yml`](./deploy_pve_monitoring.md) | Deploy Node Exporter and the Proxmox VE Exporter on Proxmox hosts for monitoring Usage: ansible-playbook -i inventory/pve/inventory.ini playbooks/proxmox/deploy_pve_monitoring.yml |
